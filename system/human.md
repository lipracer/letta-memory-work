...
M300 硬件**在编译层面兼容 CUDA**。终极目标:用**自己编译的 PyTorch + 自研 CUDA 兼容运行时**跑通上游测试。

**责任边界(2026-08-31 用户纠正,之前我写错了)**:用户负责的是 **torch.compile 这一块**,
**不是整个 PyTorch**。torch 其他模块(eager 算子、分布式、量化…)的全量测试是**团队**的。
...
用户要做 M300 PyTorch **autotune**(torch.compile;它是 93 个 feature 总盘子里用户 owner 的那一格，p0，6 个 feature。任务文档索引见 [[reference/m300/task_docs.md]]；用例数必须现场 --collect-only 取证，文档里的数不可信)的夜间批量测试:委托 agent 登录环境 → 体检磁盘 → 拉镜像建容器 → 搭环境 → 调度分片跑测试。**该套测试没有统一 runner,必须由 agent 统一调度分配。** 用户明确要求(2026-08-28):**先不跑,先把工作流定义清楚、沉淀成可复用流程**;并认同"先跑通一个例子,后面就好办"。
