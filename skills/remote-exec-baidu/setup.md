...
**新增节点探测的实证提醒（2026-09-01）**：对 26 台共享节点做批量只读体检时仅 6 台连通，其余主要返回 `banner timeout`/`relay closed`；不能据此断言机器无权限。需要将“节点自身可达性”和“relay 并发/连接复用限制”拆开，用关闭 ControlMaster 的阶梯实验（1、2、4、8、16、26）复测，并记录每档错误类型。
