...
**必须放进夜间窗口(工作日 23:00→06:00,仅 7 小时)内,超了就砍范围或分多晚接力**,
不要指望它跑得比估算快 —— 详见下面「机器资源是借来的」。
...
### 排期反推(P2 定分片时就要算)
工作日可用窗口 = **7 小时**(23:00→06:00),扣掉建容器/装环境/冒烟的前置开销,
实际跑测试的时间更短。
`用例数 × 平均耗时 × 安全系数 2` **超过窗口就砍范围**,分多晚跑,
**不要指望它跑得比估算快**,也不要靠"拖到白天"或"提前到 23:00 前"补。
周末窗口大得多,大批量优先排周末。

**一晚跑不完是常态,所以分片必须支持多晚接力**:分片粒度要小到能在一晚内跑完若干整片,
并记录"哪几片已完成",下一晚从未完成的片继续,不要从头重跑。
