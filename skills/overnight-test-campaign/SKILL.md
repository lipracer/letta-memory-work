...
**必须放进夜间窗口(20:00→06:00,约 10 小时)内,超了就砍范围**,
不要指望它跑得比估算快 —— 详见上面「机器资源是借来的」。
...
| 时段 | 可用 |
|---|---|
| 工作日 夜间(约 20:00 → **次日 06:00**) | 可用 |
| 周末 全天 | 可用 |
| 工作日 白天(06:00 → 20:00) | **禁止铺开** |
...
### 排期反推(P2 定分片时就要算)
可用窗口 ≈ **10 小时**(20:00→06:00),扣掉建容器/装环境/冒烟的前置开销,
实际跑测试的时间更短。
`用例数 × 平均耗时 × 安全系数 2` **超过窗口就砍范围**,分多晚跑,
**不要指望它跑得比估算快**,也不要靠"拖到白天"补。
周末窗口大得多,大批量优先排周末。
