...
一个脚本做不了这件事,别指望它。切分如下:

| 谁 | 管什么 |
|---|---|
| **远端脚本(每片一个)** | 单片内的硬不变量:`timeout`、日志落盘、标 `TRUNCATED`、写完成状态 |
| **我(主 agent)** | 跨机调度:派首批、收回收信号、决定下一片给谁、06:00 停止续投、判断红是环境还是真 bug |

脚本保证**单片不越界**,我保证**全局往前走**。
这也是为什么不需要 LangGraph 那类框架:调度是我的判断工作,不是状态图能替的;
而硬约束在远端 shell 里,框架也管不着。
...
⚠️ **这条与「多机小批同时铺 10 台」直接冲突,尚未解决(2026-08-30)。**
瓶颈不在远端机器,在**本机到 relay 网关**这一跳。所以"找 10 台机器同时派"
不能靠本机开 10 路 ssh 实现。可能的出路(**都未验证**):
①派发是短连接(下发即断,让远端脚本 `nohup` 自己跑),只在**收状态时**才连,
把并发从"常驻 10 条"降成"轮询时 ≤2 条";②首批分批派,错开首连认证;
③查 relay 有无更高并发的通路。
**在这条解决之前,不许按 10 台同时铺开设计 P3** —— 会整片断在 banner exchange。
