...
> ⚠️ **状态：可能已过时（2026-08-31 用户告知）**。用户说 event 返 0 这个问题
> **上游/内部已修**，我取证时用的镜像版本没更新。
> → 在新镜像上重新测一次 event 计时再引用本条；
> 若已修，则 `TORCHINDUCTOR_USE_EXPERIMENTAL_BENCHMARKER=0` 这个规避开关**不再必要**。
> **不要再拿"模拟器没有时间信息"当作性能线的阻碍结论往外说。**
> 保留本条的理由：诊断方法（“模拟器上异常慢 → 先怀疑计时返 0”）仍然有效。
