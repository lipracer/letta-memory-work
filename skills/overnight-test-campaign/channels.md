...
**最新结论(2026-09-01):HTTP 能否裸直连取决于宿主端口。node41 的 8000 与 8111 已从本机直连成功,不必经 SSH 隧道;1025/28080 曾超时。不要再概括成“裸直连都不通”。**

| 路径 | 结果 |
|---|---|
| 本机 `curl http://10.206.192.139:8000/` | ✅ HTTP 200，远端 `python3 -m http.server 8000` |
| 本机直连 Docker 映射 `10.206.192.139:8111` | ✅ GET health、POST run、GET job 全通；约 0.18~0.20s，无 SSH 隧道 |
| 本机直连动态 Docker 端口 `10.206.192.139:1025` | ❌ timeout |
| 本机直连 `10.206.192.139:28080` | ❌ timeout |
| 宿主机本地访问以上 Docker 服务 | ✅ 服务本身正常 |
| **经 `ssh -L` 隧道访问被拦端口** | ✅ GET 和 POST 都通，作为端口不开放时的兜底 |
